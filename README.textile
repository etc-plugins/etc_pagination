...
Is typically used as *pages* attribute value passed to *etc_pagination*, like this:

bc. <txp:etc_pagination pages='<txp:numpages section="example" />' />

h4. Attributes

Most of *etc_numpages* attributes come from @<txp:article />@ and other Textpattern list tags.
...
* @table="txp_table"@<br />A name of txp table from which the list will be extracted. Default: @textpattern@ (articles).
* @total="number"@<br />An optional list length, if known.
* @author@, @category@, @excerpted@, @exclude@, @expired@, @id@, @include@, @keywords@, @month@, @realname@, @section@, @status@, @time@<br />have the same meaning as in @<txp:article_custom />@ and other list tags.
...
bc. <ul>
    ...
    <li>
        <a href="http://example.com/blog/">1</a>
    </li>
    <li>
